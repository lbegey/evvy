  ...
  return (
    <MockFrame>
      <div className="space-y-2.5">
        ...
        <div className="flex h-9 items-center rounded-lg border border-border/60 bg-muted/30 px-3 text-xs text-muted-foreground">2 invités</div>
  ...
  return (
    <MockFrame>
      ...
      <div className="mt-1.5 grid grid-cols-7 gap-1.5">
        {Array.from({ length: 28 }).map((_, i) => (
          <div
            key={i}
            className={cn(
              "flex h-7 items-center justify-center rounded-md text-[11px] sm:h-8",
              filled.includes(i)
      ...
      <div className="mt-4 space-y-2">
        <div className="flex items-center gap-2 rounded-lg bg-primary/10 px-3 py-2 text-xs text-foreground">
          <span className="h-2 w-2 shrink-0 rounded-full bg-primary" />
          Lancement produit — 12 juin
        </div>
        <div className="flex items-center gap-2 rounded-lg bg-foreground/5 px-3 py-2 text-xs text-foreground">
...
export function LandingContent({ demoCalendar }: LandingContentProps) {
  ...
  const featureSections = [
    ...
    { ...T.landing.features.calendars, mockup: <CalendarsMock />, reverse: true },
