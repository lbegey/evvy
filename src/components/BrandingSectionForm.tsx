...
import { type BrandingPreset, type BrandingPresetData } from "@/app/actions/brandingPresets";
...
export type BrandingFormData = BrandingPresetData & { brandingEnabled: boolean };

interface BrandingSectionFormProps {
  targetId: string;
  idPrefix: string;
  plan: string;
  ...
  updateBranding: (id: string, data: BrandingFormData) => Promise<{ error: "forbidden" } | undefined>;
  applyPreset: (id: string, presetId: string) => Promise<{ error: "forbidden" } | undefined>;
}

export function BrandingSectionForm({
  targetId, idPrefix, plan, brandingEnabled,
  ...
  initialPresets, updateBranding, applyPreset,
}: BrandingSectionFormProps) {
  ...
  const onApplyPreset = async (preset: BrandingPreset) => {
    ...
    await applyPreset(targetId, preset.id);
    router.refresh();
  };
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex items-center justify-between gap-3">
        <Label htmlFor={`${idPrefix}-enabled`} className="cursor-pointer">{T.eventDetail.branding.enable}</Label>
        <div className="flex items-center gap-2">
          ...
          <button
            type="button" id={`${idPrefix}-enabled`} role="switch" aria-checked={enabled}
        ...
        <div className="space-y-4 border-t border-border/60 pt-4">
          ...
          {logoUrl && (
            <div className="space-y-2">
              <Label htmlFor={`${idPrefix}-logo-size`}>{T.eventDetail.branding.logoSize}</Label>
              <div className="flex items-center gap-3">
                <input
                  id={`${idPrefix}-logo-size`} type="range"
              ...
              <div className="grid gap-2 sm:grid-cols-2">
                {[
                  { id: `${idPrefix}-logo-transparent`, label: T.eventDetail.branding.logoTransparentBg, checked: logoTransparentBg, toggle: () => { const next = !logoTransparentBg; setLogoTransparentBg(next); persist({ logoTransparentBg: next }); } },
                  { id: `${idPrefix}-logo-rounded`, label: T.eventDetail.branding.logoRounded, checked: logoRounded, toggle: () => { const next = !logoRounded; setLogoRounded(next); persist({ logoRounded: next }); } },
          ...
          <div className="grid gap-3 sm:grid-cols-2">
            <BrandingColorField id={`${idPrefix}-color`} label={T.eventDetail.branding.color} value={color} placeholder="#6366f1" onChange={setColor} onBlur={() => persist({ color })} />
            <BrandingColorField id={`${idPrefix}-text`} label={T.eventDetail.branding.textColor} value={textColor} placeholder="#111111" defaultColor="#111111" onChange={setTextColor} onBlur={() => persist({ textColor })} />
            <BrandingColorField id={`${idPrefix}-bg`} label={T.eventDetail.branding.backgroundColor} value={backgroundColor} placeholder="#f4f4f5" defaultColor="#f4f4f5" onChange={setBackgroundColor} onBlur={() => persist({ backgroundColor })} />
            <BrandingColorField id={`${idPrefix}-card`} label={T.eventDetail.branding.cardColor} value={cardColor} placeholder="#ffffff" defaultColor="#ffffff" onChange={setCardColor} onBlur={() => persist({ cardColor })} />
            <BrandingColorField id={`${idPrefix}-icon-bg`} label={T.eventDetail.branding.iconBackgroundColor} value={iconBackgroundColor} placeholder="#ffffff" defaultColor="#ffffff" onChange={setIconBackgroundColor} onBlur={() => persist({ iconBackgroundColor })} />
          </div>
