...
interface EventSlugSectionProps {
  eventId: string;
  plan: string;
  appUrl: string;
  slug: string | null;
...
export function EventSlugSection({ eventId, plan, appUrl, slug }: EventSlugSectionProps) {
  ...
  return (
    <div className="space-y-1.5 border-t border-border/60 pt-3">
      <Label htmlFor="event-slug">{T.eventDetail.slug.title}</Label>
      <p className="text-xs text-muted-foreground">{T.eventDetail.slug.subtitle}</p>
      <div className="flex flex-wrap items-center gap-1.5">
        <span className="break-all text-xs text-muted-foreground">{appUrl}/e/</span>
        <Input
          id="event-slug"
          value={value}
